...
class Meta_Trainer(nn.Module):
    ...
    # feed the data into model and get prediction result
    def get_prediction(self, model, data, taskid,epoch,train,message):
        '''
        param model : A model used to predict the result
        param data : Data involved in model training and prediction
        param taskid: Define which task
        param train: if trian
        param message: if use property-guided feature augmentatin
        '''
        if train:
            if message:
                s_logits, q_logits, subs_logits,subq_logits,cats_logits,catq_logits,pos_emb = model(taskid,data['s_data'], data['q_data'],data['f_tasks_pos'],data['s_label'])
                ...
                s_logits, q_logits, subs_logits,subq_logits,cats_logits,catq_logits,pos_emb = model(taskid,data['s_data'], data['q_data'],None,data['s_label'])
                pred_dict = {'s_logits': s_logits, 'q_logits': q_logits,'subs_logits':subs_logits,'subq_logits':subq_logits,'cats_logits':cats_logits,'catq_logits':catq_logits,'pos_emb':pos_emb}
        else:
            s_logits, logits, labels = model.forward_query_loader(taskid,epoch,data['s_data'],data['data_loader'],data['s_label'])
    ...
    def get_loss(self, model, batch_data, pred_dict, train=True, flag=0):
        '''
        param model : A model used to predict the result
        param batch_data : Data involved in model training and prediction
        param pred_dict: prediction results
        param train: if trian
        param flag: 0: query 1: support
        '''
        ...
        for k in range(self.update_step):
            ...
            for task_id in task_id_list:
                ...
                for inner_step in range(self.inner_update_step):
                    pred_adapt = self.get_prediction(model, train_data,task_id,epoch,train=True,message=False)
    ...
    def test_step(self,epoch,f_tasks):
        ...
        for task_id in range(len(self.test_tasks)):
            ...
            if self.update_step_test > 0:
                ...
                for i, batch in enumerate(adapt_data['data_loader']):
                    ...
                    adaptable_weights = self.get_adaptable_weights(model)
                    pred_adapt = self.get_prediction(model, cur_adapt_data, task_id,epoch,train=True,message=True)
            ...
            model.eval()
            with torch.no_grad():
                pred_eval = self.get_prediction(model, eval_data,task_id, epoch,train=False,message=False)
    ...
    def save_model(self):
        ...
        save_path = os.path.join(self.trial_path, "pg_dern.pth")
