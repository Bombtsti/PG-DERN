...
class GNN_Encoder(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim,  JK="last", drop_ratio=0, graph_pooling="mean", gnn_type="gin",batch_norm=True):
        ...
        self.encoder = SubgraphEncoder(embed_dim=emb_dim,gnn_type=gnn_type,k_hop=2)
        # use attention?
    ...
    def forward(self, *argv):
        ...
        # attention_scores = F.softmax(torch.matmul(output, output.t()), dim=-1)
        # output = torch.matmul(attention_scores, output)

        # learnable param
        # graph_representation = self.pool(node_representation, batch)
        # subg_representation = self.pool(output,batch)
        # weight = self.attention(graph_representation,subg_representation)
        # weight1 = torch.chunk(weight,2,dim=1)[0]
        # weight2 = torch.chunk(weight,2,dim=1)[1]
        # gs_representation = weight1*graph_representation+weight2*subg_representation
...
class GCN(torch.nn.Module):
    ...
    def forward(self,A,X):
        ...
        A_norm=A-1e9*torch.less_equal(A,0.7)
