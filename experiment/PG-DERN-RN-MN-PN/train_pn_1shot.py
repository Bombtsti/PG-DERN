...
class ProtoNetwork(nn.Module):
    ...
    def forward(self,x):
        ...
        z = self.encoder(x)
        z = z.view(z.size(0), -1)
        z_proto = z[:2 * 1].view(2, 1, -1).mean(1)
        zq = z[2 * 1:]
...
def main():
    sys.stdout = Logger('resultpn1shot.txt')
    args = get_args('.')
    ...
    feature_encoder = GNN_Encoder(num_layer=5, emb_dim=300, JK='last',
                                       ...
                                       batch_norm = 1)
    relation_network = ProtoNetwork(300,128)
    ...
    if os.path.exists(str("./models/feature_encoder_" + str(2) +"way_" + str(1) +"shot.pkl")):
        feature_encoder.load_state_dict(torch.load(str("./models/pn1/feature_encoder_" + str(2) +"way_" + str(1) +"shot.pkl")))
        print("load feature encoder success")
    if os.path.exists(str("./models/relation_network_"+ str(2) +"way_" + str(1) +"shot.pkl")):
        relation_network.load_state_dict(torch.load(str("./models/pn1/proto_network_"+ str(2) +"way_" + str(1) +"shot.pkl")))
        print("load relation network success")
    if os.path.exists(str("./models/message_gcn_"+ str(2) +"way_" + str(1) +"shot.pkl")):
        message_gcn.load_state_dict(torch.load(str("./models/pn1/message_gcn_"+ str(2) +"way_" + str(1) +"shot.pkl")))
    ...
    for episode in range(EPISODE):
        ...
        for task in task_id_list:
            db = task_generator.MetaTask(task,preload_train_data,'train',2,1,16)
        ...
        if episode%100 == 0:
            ...
            for task in task_id_list:
                db = task_generator.MetaTask(task, preload_test_data,'test', 2, 1, 16)
            ...
            if avg_scores > last_scores:
                # save networks
                torch.save(feature_encoder.state_dict(),str("./models/pn1/feature_encoder_" + str(2) +"way_" + str(1) +"shot.pkl"))
                torch.save(relation_network.state_dict(),str("./models/pn1/proto_network_"+ str(2) +"way_" + str(1) +"shot.pkl"))
                torch.save(message_gcn.state_dict(),str("./models/pn1/message_gcn_" + str(2) + "way_" + str(1) + "shot.pkl"))
