...
from chem_lib.datasets.data import GraphDataset
...
def sample_meta_datasets(data, dataset, task, n_shot, n_query):
    ...
    s_list_1 = neg_sample[:n_shot]
    s_list_2 = pos_sample[:n_shot]

    l = [i for i in range(0, len(data)) if i not in s_list_1+s_list_2]
    random.shuffle(l)

    q_sample = sample_inds(l, n_query)
    q_list = q_sample[:n_query]
    return s_list_1+s_list_2+q_list

def sample_test_datasets(data, dataset, task, n_shot, n_query):
    distri_list = obtain_distr_list(dataset)
    thresh = distri_list[task][0]
    ...
    q_list = [i for i in range(0, len(data)) if i not in s_list_1+s_list_2]
...
class MetaTask(object):
    def __init__(self,task,preload_data,tag,num_ways,support_num,query_num):
        ...
        if task in preload_data:
            dataset = preload_data[task]
        else:
            dataset = MoleculeDataset("/data/tox21/new/" + str(task + 1), dataset='tox21')
        # print(c)
        if tag == 'train':
            samples[task] = sample_meta_datasets(dataset, 'tox21', task, self.support_num, self.query_num)
            self.support_samples[task] = samples[task][:support_num * 2]
            self.query_samples[task] = samples[task][support_num * 2:]

            support_data = GraphDataset(dataset[samples[task][:support_num * 2]], degree=False, k_hop=2, se="khopgnn",
                                        use_subgraph_edge_attr=True)
            query_data = GraphDataset(dataset[samples[task][support_num * 2:]], degree=False, k_hop=2, se="khopgnn",
                                      use_subgraph_edge_attr=True)

            self.support_loader[task] = loader_to_samples(support_data)
            self.query_loader[task] = loader_to_samples(query_data)

            self.support_labels[task] = loader_to_samples(support_data).y
            self.query_labels[task] = loader_to_samples(query_data).y

        else :
            samples[task] = sample_test_datasets(dataset, 'tox21', task, self.support_num, self.query_num)
            self.support_samples[task] = samples[task][:support_num*2]
            self.query_samples[task] = samples[task][support_num*2:]

            support_data = GraphDataset(dataset[samples[task][:support_num*2]], degree=False, k_hop=2, se="khopgnn",use_subgraph_edge_attr=True)
            query_data = GraphDataset(dataset[samples[task][support_num*2:]], degree=False,k_hop=2, se="khopgnn", use_subgraph_edge_attr=True)

            self.support_loader[task] = loader_to_samples(support_data)
            self.query_loader[task] = DataLoader(query_data, batch_size=query_num, shuffle=True, num_workers=0)

            self.support_labels[task] = loader_to_samples(support_data).y
            self.query_labels[task] = []
