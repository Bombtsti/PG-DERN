...
class TaskAwareRelation(nn.Module):
    def __init__(self, inp_dim, hidden_dim, num_layers, edge_n_layer, num_class=2,
        ...
        # self.fc1 = nn.Sequential(nn.Linear(300, 300), nn.LeakyReLU())
        # self.fc2 = nn.Sequential(nn.Linear(300, 300), nn.LeakyReLU())
        # self.fc3 = nn.Sequential(nn.Linear(300, 128), nn.LeakyReLU())
        self.fc2 = nn.Linear(300, num_class)
        # assert 0 <= res_alpha <= 1

    def forward(self, s_emb, q_emb, return_adj=False, return_emb=False):
        # node_feat=all_emb
        # s_emb = self.fc3(self.fc2(self.fc1(s_emb)))
        # q_emb = self.fc3(self.fc2(self.fc1(q_emb)))
        # edge_feat_list = []
        # if return_adj:
        #     x_i = node_feat.unsqueeze(2)
        #     x_j = torch.transpose(x_i, 1, 2)
