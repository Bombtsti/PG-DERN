...
from .encoder import GNN_Encoder, GCN1
from .relation import ContextMLP, TaskAwareRelation
...
class ContextAwareRelationNet(nn.Module):
    def __init__(self, args):
        ...
        # 特征增强模块
        self.message_gcn1 = GCN1(args.emb_dim, args.emb_dim)
        # self.message_gcn2 = GCN2(args.emb_dim, args.emb_dim)
        inp_dim = args.map_dim
        self.adapt_relation = TaskAwareRelation(inp_dim=inp_dim,num_class=2,pre_dropout=args.rel_dropout2)
        self.squeeze_liner = torch.nn.Linear(300, 128)
    ...
    def ftask_aug(self,s_emb, f_tasks_pos):
        ...
        new_pos_emb = self.message_gcn1(similarity, fc_embs)
    ...
    def fea_aug(self,s_emb_map):
        ...
        #     sq_emb = self.squeeze_liner(emb)
        #     x = sq_emb / torch.norm(sq_emb, dim=-1, keepdim=True)  # 方差归一化，即除以各自的模
        #     similarity = torch.mm(x, x.T)  # 矩阵乘法
        #     new_emb = self.message_gcn1(similarity,emb)
        #     list.append(new_emb)
        ...
        sq_emb = self.squeeze_liner(emb)
        x = sq_emb / torch.norm(sq_emb, dim=-1, keepdim=True)  # 方差归一化，即除以各自的模
        soft_adj = torch.mm(x, x.T)  # 矩阵乘法
        new_emb = self.message_gcn1(soft_adj, emb)
        s_emb_map = new_emb.reshape((q, s, d))
        return s_emb_map

    def forward(self, s_data, q_data,f_tasks_pos, s_label=None):
